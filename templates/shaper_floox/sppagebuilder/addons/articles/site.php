...
class SppagebuilderAddonArticles extends SppagebuilderAddons {
    ...
    public function render() {
        ...
        $version_selector = (isset($this->addon->settings->version_selector) && $this->addon->settings->version_selector) ? $this->addon->settings->version_selector : 'article';
        ...
        if ($version_selector == 'flipbox') {
          ...
          if (count($items)) {
              ...
              foreach ($items as $key => $item) {
                  $output .= '<div class="sppb-col-sm-' . round(12 / $columns) . ' '.$class.'">';
                  ...
                  if ($show_author || $show_date) {
                      ...
                      if ($show_date) {
                          $output .= '<i class="floox-icon floox-clock"></i> <span class="sppb-meta-date" itemprop="dateCreated">' . Jhtml::_('date', $item->created, 'DATE_FORMAT_LC3') . '</span>';
                      }

                      if ($show_author) {
                          $output .= '<i class="fa fa-user-o"></i> <span class="sppb-meta-author" itemprop="name">' . $item->username . '</span>';
                      }
                      if ($show_hits) {
                          $output .= '<i class="fa fa-eye"></i> <span class="sppb-meta-hits" itemprop="hits">' . $item->hits . '</span>';
                      }
                  ...
                  if ($show_intro) {
                      $output .= '<p>' . Jhtml::_('string.truncate', ($item->introtext), $intro_limit) . '</p>';
                  }
          ...
          if (count($items)) {
              ...
              foreach ($items as $key => $item) {
                  ...
                  $output .= '<div class="sppb-addon-article-bgimage" style="background-image: url(' . $item->image_thumbnail . ');">';
                  $output .= '</div>'; //.sppb-addon-article-bgimage

                  $output .= '<div class="sppb-addon-article-info-wrap">';
                  ...
                  if ($show_readmore) {
                      $output .= '<a class="sppb-readmore btn btn-primary" href="' . $item->link . '" itemprop="url">' . $readmore_text . '</a>';
                  }

                  $output .= '</div>'; //.sppb-addon-article-info-wrap
                  $output .= '</div>';
                  $output .= '</div>';
              }
